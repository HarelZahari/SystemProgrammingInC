...
int expMod_efficient(int a, int exp, int n){
    if(a<=0 || n<=0 || exp <0){
        ...
        int posMSB=getMSBpos(exp);
        int Ak=a;
        int multModResult=1;
        int currentIndex=getNextBitPos(exp);
        for(int i=0;i<=posMSB;i++){
            if(currentIndex==i){
                multModResult=multModulo(Ak,multModResult,n);
                currentIndex=getNextBitPos(exp);
            }
...
int findFermatWitness(int n, int numReps){
 ...
 for(int i=0;i<numReps;i++){
     int currentN=(rand() % (n-3))+2;
     int x=expMod_efficient(currentN,n-1,n);
     if(x!=1){
         return currentN;
     }
 }
...
int main () {
    ...
    if (x < 2147483647 && x > -(2147483647)){
        ...
        if(witnessRes!=0){
            ...
            printf("The number %d is potentially prime. No Fermat witness found.\n",a);
        }
    }else{
        printf("Not a valid positive int\n");
        return 1;
